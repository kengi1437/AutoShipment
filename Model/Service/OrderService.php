...
class OrderService
{
    ...
    public function processShipanyResult($result)
    {
        ...
        debug_time('processShipanyResult');
        // debug_time($result);
        debug_time($trackList);
        ...
        foreach ($trackList as $trackItem) {
            if ($trackItem['increment_id'] && $trackItem['track_no']) {
                ...
                // echo $order->getShipanyStatus() . PHP_EOL;
                // print_r($trackItem);
                debug_time($order->getId() . ' ' . $order->getState() . ' ' . $order->getStatus() . ' ' . $order->getShipanyStatus());
                debug_time($order->getStatus() == 'processing' ? 'processing' : '');
                debug_time($trackItem['status'] == 'shipped' ? 'shipped' : '');
                ...
                if ($order->getStatus() == 'complete' && empty($trackItem['status'])) {
                    ...
                    debug_time('processing complete');
                    // $this->processOrder($order);
                    $this->processOrderShipment($order);
                    debug_time($order->getId() . ' ' . $order->getState() . ' ' . $order->getStatus() . ' ' . $order->getShipanyStatus());
                    ...
                    try {
                        ...
                        debug_time('order complete email success');
                    } catch (\Exception $e) {
                        debug_time('order complete email fail');
                    }
